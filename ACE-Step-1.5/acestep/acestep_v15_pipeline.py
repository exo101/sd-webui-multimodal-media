...
# ============================================
# FFmpeg 路径配置（必须在加载 torchaudio 之前执行）
# ============================================
def _configure_ffmpeg_path():
    """配置 FFmpeg 路径以确保 torchcodec 和 torchaudio 可以找到 FFmpeg"""
    ...
    webui_root = Path(_project_root).parent.parent.parent  # 向上追溯到 webui 目录
    ffmpeg_paths.extend([
    ...
    for ffmpeg_path in ffmpeg_paths:
        ffmpeg_path_str = str(ffmpeg_path)
        if os.path.exists(ffmpeg_path_str):
            # 检查 ffmpeg 和 ffprobe 是否存在
            if os.name == 'nt':
                ffmpeg_exe = os.path.join(ffmpeg_path_str, "ffmpeg.exe")
                ffprobe_exe = os.path.join(ffmpeg_path_str, "ffprobe.exe")
            else:
                ffmpeg_exe = os.path.join(ffmpeg_path_str, "ffmpeg")
                ffprobe_exe = os.path.join(ffmpeg_path_str, "ffprobe")
            
            if os.path.exists(ffmpeg_exe) and os.path.exists(ffprobe_exe):
                # 将路径添加到系统 PATH 最前面
                if system_path:
                    os.environ["PATH"] = ffmpeg_path_str + os.pathsep + system_path
                else:
                    os.environ["PATH"] = ffmpeg_path_str
                print(f"[FFmpeg配置] ✓ 已找到 FFmpeg: {ffmpeg_path_str}")
                return True
    
    print("[FFmpeg配置] ⚠️ 未找到 FFmpeg，音频处理功能可能受限")
    return False

# 执行 FFmpeg 配置（必须在加载 torchaudio 之前）
