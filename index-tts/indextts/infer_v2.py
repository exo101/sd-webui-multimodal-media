...
class IndexTTS2:
    def __init__(
        ...
        self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(
            os.path.join(self.model_dir, "w2v-bert-2.0", "AI-ModelScope", "w2v-bert-2___0"),
            local_files_only=True,
            cache_dir=os.path.join(self.model_dir, "w2v-bert-2.0")
        )
        ...
        semantic_code_ckpt = os.path.join(self.model_dir, "MaskGCT", "AI-ModelScope", "MaskGCT", "semantic_codec", "model.safetensors")
        ...
        # )
        campplus_ckpt_path = os.path.join(self.model_dir, "campplus", "funasr", "campplus", "campplus_cn_common.bin")
        ...
        # 使用本地模型路径而不是从HuggingFace下载
        local_bigvgan_path = os.path.join(self.model_dir, "bigvgan_v2_22khz_80band_256x")
        try:
            if os.path.exists(local_bigvgan_path):
                print(f"Loading BigVGAN from local path: {local_bigvgan_path}")
                self.bigvgan = bigvgan.BigVGAN.from_pretrained(local_bigvgan_path, use_cuda_kernel=self.use_cuda_kernel)
            else:
                print(f"Local BigVGAN not found, trying to load from HuggingFace: {bigvgan_name}")
                self.bigvgan = bigvgan.BigVGAN.from_pretrained(bigvgan_name, use_cuda_kernel=self.use_cuda_kernel)
        except Exception as e:
            print(f"Failed to load BigVGAN from both local and remote, trying with local_files_only=True: {e}")
