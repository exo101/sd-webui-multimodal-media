...
def _resolve_paths(pretrained_path: str, version: str):
    ...
    # 修改tokenizer和gen_config的路径，从HeartTranscriptor-oss目录查找
    transcriptor_path = os.path.join(pretrained_path, "HeartTranscriptor-oss")
    tokenizer_path = os.path.join(transcriptor_path, "tokenizer.json")
    gen_config_path = os.path.join(transcriptor_path, "generation_config.json")
    ...
    if not os.path.isfile(tokenizer_path):
        raise FileNotFoundError(
            f"Expected to find tokenizer.json for HeartMuLa at {tokenizer_path} but not found. Please check your folder {transcriptor_path}."
        )
    if not os.path.isfile(gen_config_path):
        raise FileNotFoundError(
            f"Expected to find gen_config.json for HeartMuLa at {gen_config_path} but not found. Please check your folder {transcriptor_path}."
        )
